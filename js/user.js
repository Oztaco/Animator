...
surface('timelineCanvas', { dpiScaling: 'live' });
surface('sideBarPalette', { dpiScaling: 'live' });
...
scene("_loader", {
	...
	update: function() {
		...
		if (downloadDone || true) setScene("intro"); // TODO: Bug when 0 downloads
		else {
...
scene("intro", {
	init: function() {
		...
		frameContainer = createFrameHolder(3, canvas, ctx);
...
scene("draw", {
	...
	update: function() {
		...
		var offset = getOffsetOf(canvas);
		if (mouse.leftButton && prevDot.x == null) {
			prevDot.x = (mouse.x - offset.left) * 2;
			prevDot.y = (mouse.y - offset.top) * 2.1;
		}
		if (mouse.leftButton) {
			var tmpCtx = frameContainer.list[frameContainer.selected].image.getContext("2d");
			// tmpCtx.fillStyle = "#000000";
			// tmpCtx.fillRect(Math.round((mouse.x - offset.left) * 2), Math.round((mouse.y - offset.top) * 2.05), 2, 2);
			tmpCtx.strokeStyle = preferences.penColor.color;
			tmpCtx.lineWidth = preferences.penSize;
			tmpCtx.lineCap = "round";
			tmpCtx.beginPath();
			tmpCtx.moveTo(prevDot.x, prevDot.y);
			tmpCtx.lineTo((mouse.x - offset.left) * 2, (mouse.y - offset.top) * 2.1);
			tmpCtx.stroke();
			prevDot.x = (mouse.x - offset.left) * 2;
			prevDot.y = (mouse.y - offset.top) * 2.1;
		}
	...
	draw: function() {
		// Color palette
		renderColorPicker(surfaces['sideBarPalette']);
		...
		renderCanvasPreview(frameContainer, canvas, ctx);
		renderTimeline(frameContainer, timelineCanvas, canvas, timelineCtx);
...
function init() {
	startEngine(pipeline, "drawingCanvas");
	requestAnimationFrame(pipeline);
}


var timelineScroll = 0;
var timelineThumbScale = 0.83;
var timelineAspectRatio = 1 / 0.5625;
var timelineThumbSpacing = 30;
function renderTimeline(frames, timelineCanvas, canvas, ctx) {
	var centerX = timelineCanvas.width / 2;
	var thumbPxWidth = timelineCanvas.height * timelineThumbScale * timelineAspectRatio;
	var thumbPxHeight = timelineCanvas.height * timelineThumbScale;
	timelineScroll = frames.selected * (thumbPxWidth + timelineThumbSpacing);
	...
	ctx.fillRect(
		centerX - (timelineThumbSpacing / 2),
		0,
		thumbPxWidth + timelineThumbSpacing,
		thumbPxHeight + timelineThumbSpacing
	...
	for (var i = 0; i < frames.list.length; i++) {
		// Background
		ctx.fillRect(
			((i * thumbPxWidth) - timelineScroll) + (i * timelineThumbSpacing) + centerX,
			timelineThumbSpacing / 2,
			thumbPxWidth,
			thumbPxHeight
		);
		// Image
...
function renderCanvasPreview(frames, canvas, ctx) {
	if (frames.list[frames.selected - 2]) {
		ctx.globalAlpha = 0.02;
		ctx.drawImage(frames.list[frames.selected - 2].image, 0, 0, canvas.width, canvas.height);
	}
	if (frames.list[frames.selected - 1]) {
		ctx.globalAlpha = 0.1;
		ctx.drawImage(frames.list[frames.selected - 1].image, 0, 0, canvas.width, canvas.height);
	}
	ctx.globalAlpha = 1;
	ctx.drawImage(frames.list[frames.selected].image, 0, 0, canvas.width, canvas.height);
}

function renderCanvasFinal(frames, canvas, ctx) {
	ctx.globalAlpha = 1;
	ctx.drawImage(frames.list[frames.selected].image, 0, 0, canvas.width, canvas.height);
}


function createFrameHolder(numberOfFrames, canvas, ctx) {
	var frames = {
		list: [],
		selected: 0
	};

	frames.addBlankFrameAt = function(index, canvas) {
		this.list.insert(index,
			{
				image: document.createElement('canvas')
			}
		);
		this.list[index].image.height = canvas.height * 1;
		this.list[index].image.width = canvas.width * 1;
	}

	for (var i = 0; i < numberOfFrames; i++) {
		frames.addBlankFrameAt(0, canvas);
	}
	return frames;
}

function addBlankFrameAtEnd(frameList) {
	addBlankFrameAt(frameList, frameList.list.length, document.getElementById("drawingCanvas"));
}

function addBlankFrameAt(frameList, index, canvas) {
	frameList.list.insert(index,
		{
			image: document.createElement('canvas')
		}
	);
	frameList.list[index].image.height = canvas.height * 1;
	frameList.list[index].image.width = canvas.width * 1;
}




function togglePlayAnime() {
	if (sceneVars.current.id == "draw") {
		setScene("animate");
	}
	else {
		setScene("draw");
...
scene("animate", {
	...
	draw: function() {
		...
		renderCanvasFinal(frameContainer, canvas, ctx);
		renderTimeline(frameContainer, timelineCanvas, canvas, timelineCtx);
...
function selectNextFrame() {
	frameContainer.selected++;
	if (frameContainer.selected >= frameContainer.list.length) {
		addBlankFrameAtEnd(frameContainer);
	}
}

function selectPrevFrame() {
	frameContainer.selected--;
	if (frameContainer.selected < 0) {
		frameContainer.selected = frameContainer.list.length - 1;
	}
}

function deleteSelectedFrame(frameList) {
	if (frameList.list.length == 1) {
		addBlankFrameAtEnd(frameList);
	}
	frameList.list.splice(frameList.selected, 1);
	frameList.selected--;
	if (frameList.selected < 0) {
		frameList.selected = 0;
	}
}

function addFrameAfterSelected(frameContainer) {
	frameContainer.addBlankFrameAt(frameContainer.selected + 1, document.getElementById('drawingCanvas'));
	frameContainer.selected++;
}

function addFrameBeforeSelected(frameContainer) {
	frameContainer.addBlankFrameAt(frameContainer.selected, document.getElementById('drawingCanvas'));
	// frameContainer.selected++;
}

function dupFrameAfterSelected(frameContainer) {
	frameContainer.addBlankFrameAt(frameContainer.selected + 1, document.getElementById('drawingCanvas'));
	frameContainer.selected++;
	var ctx = frameContainer.list[frameContainer.selected].image.getContext("2d");
	ctx.drawImage(
		frameContainer.list[frameContainer.selected - 1].image, 0, 0,
		config.canvas.resolution.width,
		config.canvas.resolution.height,
	)
}

function dupFrameBeforeSelected(frameContainer) {
	frameContainer.addBlankFrameAt(frameContainer.selected, document.getElementById('drawingCanvas'));
	frameContainer.selected;
	var ctx = frameContainer.list[frameContainer.selected].image.getContext("2d");
	ctx.drawImage(
...
function renderColorPicker(surface) {
	var size = surface.element.width / colorSchemes[0].rows.length;
	for (var r = 0; r < colorSchemes[0].rows.length; r++) {
		for (var c = 0; c < colorSchemes[0].rows[0].length; c++) {
			surface.ctx.fillStyle = colorSchemes[0].rows[r][c];
			surface.ctx.fillRect(r * size, c * size, size, size);
		}
	}
	// Draw selected color
	if (preferences.penColor.row != null) {
		surface.ctx.lineWidth = "3";

		surface.ctx.beginPath();
		surface.ctx.strokeStyle = "#fff";
		surface.ctx.rect(
			preferences.penColor.row * size + 2,
			preferences.penColor.col * size + 2,
			size - 4, size - 4
		);
		surface.ctx.stroke();

		surface.ctx.beginPath();
		surface.ctx.strokeStyle = "#000";
		surface.ctx.rect(
			preferences.penColor.row * size,
			preferences.penColor.col * size,
			size, size
		);
		surface.ctx.stroke();
	}
...
function setPenColor(e, self) {
	var rect = e.target.getBoundingClientRect();
	var x = e.clientX - rect.left;
	var y = e.clientY - rect.top;

	var row, col, color;
	row = Math.floor(x / self.clientWidth * colorSchemes[0].rows.length);
	col = Math.floor(y / self.clientHeight * colorSchemes[0].rows[0].length);
	color = colorSchemes[0].rows[row][col];
	preferences.penColor.row = row;
	preferences.penColor.col = col;
	preferences.penColor.color = color;
	console.log(color);
}
